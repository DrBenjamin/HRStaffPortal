...
if check_password():
    ...
    databank_drivers = pd.DataFrame(
        columns = ['ID', 'EMPLOYEE_NO', 'DRIVER_ID', 'DRIVER_FORENAME', 'DRIVER_SURNAME', 'DRIVER_NATIONAL_ID',
                   'DRIVER_MOBILE_NO', 'DRIVER_LICENSE_NO', 'DRIVER_LICENSE_CLASS', 'DRIVER_LICENSE_EXPIRY_DATE',
                   'DRIVER_PSV_BADGE', 'DRIVER_NOTES', 'DRIVER_IMAGE'])

    # Populate dataframe
    for row in rows:
        df = pd.DataFrame([[row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10],
                            row[11], row[12]]],
                          columns = ['ID', 'EMPLOYEE_NO', 'DRIVER_ID', 'DRIVER_FORENAME', 'DRIVER_SURNAME',
                                     'DRIVER_NATIONAL_ID', 'DRIVER_MOBILE_NO', 'DRIVER_LICENSE_NO',
                                     'DRIVER_LICENSE_CLASS', 'DRIVER_LICENSE_EXPIRY_DATE', 'DRIVER_PSV_BADGE',
                                     'DRIVER_NOTES', 'DRIVER_IMAGE'])
    ...
    # Create pandas dataframe
    databank_fuel = pd.DataFrame(
        columns = ['ID', 'VEHICLE_ID', 'DRIVER_ID', 'FUEL_AMOUNT', 'FUEL_COST', 'FUEL_TYPE', 'FUEL_DATE',
                   'FUEL_DISTANCE', 'FUEL_SHORTAGE', 'COST_CENTRE'])

    # Populate dataframe
    for row in rows:
        df = pd.DataFrame([[row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9]]],
                          columns = ['ID', 'VEHICLE_ID', 'DRIVER_ID', 'FUEL_AMOUNT', 'FUEL_COST', 'FUEL_TYPE',
                                     'FUEL_DATE', 'FUEL_DISTANCE', 'FUEL_SHORTAGE', 'COST_CENTRE'])
    ...
    # Create pandas dataframe
    databank_insurances = pd.DataFrame(
        columns = ['ID', 'VEHICLE_ID', 'INSURANCE_DETAILS', 'INSURANCES_TYPE', 'INSURANCE_START_DATE',
    ...
    # Create pandas dataframe
    databank_repairs = pd.DataFrame(
        columns = ['ID', 'VEHICLE_ID', 'REPAIR_DETAILS', 'REPAIR_DATE', 'REPAIR_COSTS', 'REPAIR_SPARE_PARTS',
                   'REPAIR_DOWN_TIME', 'REPAIR_CENTRE_PERSON', 'COST_CENTRE'])

    # Populate dataframe
    for row in rows:
        df = pd.DataFrame([[row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8]]],
                          columns = ['ID', 'VEHICLE_ID', 'REPAIR_DETAILS', 'REPAIR_DATE', 'REPAIR_COSTS',
                                     'REPAIR_SPARE_PARTS', 'REPAIR_DOWN_TIME', 'REPAIR_CENTRE_PERSON', 'COST_CENTRE'])
    ...
    # Create pandas dataframe
    databank_services = pd.DataFrame(
        columns = ['ID', 'VEHICLE_ID', 'DRIVER_ID', 'SERVICE_DATE', 'SERVICE_DETAILS', 'SERVICE_COSTS',
                   'SERVICE_MILEAGE_ON_SERVICE', 'SERVICE_MILEAGE_NEXTSERVICE', 'SERVICE_MILEAGE_NEXTSERVICE_MAX'])

    # Populate dataframe
    for row in rows:
        df = pd.DataFrame([[row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8]]],
                          columns = ['ID', 'VEHICLE_ID', 'DRIVER_ID', 'SERVICE_DATE', 'SERVICE_DETAILS',
                                     'SERVICE_COSTS', 'SERVICE_MILEAGE_ON_SERVICE', 'SERVICE_MILEAGE_NEXTSERVICE',
    ...
    # Create pandas dataframe
    databank_trips = pd.DataFrame(
        columns = ['ID', 'VEHICLE_ID', 'DRIVER_ID', 'TRIP_DATE', 'TRIP_DESCRIPTION', 'TRIP_COMMENTS', 'TRIP_TIME_OUT',
                   'TRIP_TIME_IN', 'TRIP_OPEN_MILEAGE', 'TRIP_CLOSE_MILEAGE', 'TRIP_DISTANCE'])

    # Populate dataframe
    for row in rows:
        df = pd.DataFrame([[row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10]]],
                          columns = ['ID', 'VEHICLE_ID', 'DRIVER_ID', 'TRIP_DATE', 'TRIP_DESCRIPTION', 'TRIP_COMMENTS',
                                     'TRIP_TIME_OUT', 'TRIP_TIME_IN', 'TRIP_OPEN_MILEAGE', 'TRIP_CLOSE_MILEAGE',
                                     'TRIP_DISTANCE'])
    ...
    databank_vehicles = pd.DataFrame(
        columns = ['ID', 'VEHICLE_ID', 'VEHICLE_PLATE_NUMBER', 'VEHICLE_TYPE', 'VEHICLE_BRAND', 'VEHICLE_MODEL',
                   'VEHICLE_SEATS', 'VEHICLE_FUEL_TYPE', 'VEHICLE_FUEL_CAPACITY', 'VEHICLE_COLOUR',
                   'VEHICLE_CHASIS_NUMBER', 'VEHICLE_MANUFACTURE_YEAR', 'VEHICLE_PURCHASE_DATE',
                   'VEHICLE_PURCHASE_PRICE', 'VEHICLE_DISPOSITION_YEAR', 'VEHICLE_COF_EXPIRY_DATE', 'VEHICLE_VENDOR',
                   'VEHICLE_DUTY', 'VEHICLE_COST_KM', 'VEHICLE_IMAGE'])
    ...
    for row in rows:
        df = pd.DataFrame([[row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10],
                          ...
                          columns = ['ID', 'VEHICLE_ID', 'VEHICLE_PLATE_NUMBER', 'VEHICLE_TYPE', 'VEHICLE_BRAND',
                                     'VEHICLE_MODEL', 'VEHICLE_SEATS', 'VEHICLE_FUEL_TYPE', 'VEHICLE_FUEL_CAPACITY',
                                     'VEHICLE_COLOUR', 'VEHICLE_CHASIS_NUMBER', 'VEHICLE_MANUFACTURE_YEAR',
                                     'VEHICLE_PURCHASE_DATE', 'VEHICLE_PURCHASE_PRICE', 'VEHICLE_DISPOSITION_YEAR',
    ...
    with st.form("Car Fleet Management", clear_on_submit = True):
        ...
        if (f"{chosen_id}" == '1'):
            ...
            if submitted:
                # Get latest ID from database
                id = lastID(url = '`carfleet`.`DRIVERS`')
                query = "INSERT INTO `carfleet`.`DRIVERS`(ID, EMPLOYEE_NO, DRIVER_ID, DRIVER_FORENAME, DRIVER_SURNAME, DRIVER_NATIONAL_ID, DRIVER_MOBILE_NO, DRIVER_LICENSE_NO, DRIVER_LICENSE_CLASS, DRIVER_LICENSE_EXPIRY_DATE, DRIVER_PSV_BADGE, DRIVER_NOTES) VALUES (%s, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');" % (
                id, employee_no, driver_id, driver_forename, driver_surname, driver_national_id, driver_mobile_no,
                driver_license_no, driver_license_class, driver_license_expiry_date, driver_psv_badge, driver_notes)
                run_query(query)
                conn.commit()
        ...
        ## tab `Fuel Consumption`
            ...
            if submitted:
                # Get latest ID from database
                id = lastID(url = '`carfleet`.`FUEL`')
                query = "INSERT INTO `carfleet`.`FUEL`(ID, VEHICLE_ID, DRIVER_ID, FUEL_AMOUNT, FUEL_COST, FUEL_TYPE, FUEL_DATE, FUEL_DISTANCE, FUEL_SHORTAGE, COST_CENTRE) VALUES (%s, '%s', '%s', %s, %s, '%s', '%s', %s, '%s', '%s');" % (
                id, vehicle_id, driver_id, fuel_amount, fuel_cost, fuel_type, fuel_date, fuel_distance, fuel_shortage,
                cost_centre)
                run_query(query)
                conn.commit()
                st.experimental_rerun()
        ...
        ## tab `Insurances`
            ...
            if submitted:
                # Get latest ID from database
                id = lastID(url = '`carfleet`.`INSURANCES`')
                query = "INSERT INTO `carfleet`.`INSURANCES`(ID, VEHICLE_ID, INSURANCE_DETAILS, INSURANCES_TYPE, INSURANCE_START_DATE, INSURANCE_EXPIRY_DATE) VALUES (%s, '%s', '%s', '%s', '%s', '%s');" % (
                id, vehicle_id, insurance_details, insurance_type, insurance_start_date, insurance_expiry_date)
                run_query(query)
                conn.commit()
                st.experimental_rerun()
        ...
        ## tab `Repairs`
            ...
            if submitted:
                # Get latest ID from database
                id = lastID(url = '`carfleet`.`REPAIRS`')
                query = "INSERT INTO `carfleet`.`REPAIRS`(ID, VEHICLE_ID, REPAIR_DETAILS, REPAIR_DATE, REPAIR_COSTS, REPAIR_SPARE_PARTS, REPAIR_DOWN_TIME, REPAIR_CENTRE_PERSON, COST_CENTRE) VALUES (%s, '%s', '%s', '%s', %s, '%s', '%s', '%s', '%s');" % (
                id, vehicle_id, repair_details, repair_date, repair_costs, repair_spare_parts, repair_down_time,
        ...
        ## tab `Services`
            ...
            service_mileage_nextservice = st.text_input(label = 'Next service mileage',
                                                        placeholder = 'Next service mileage?')
            service_mileage_nextservice_max = st.text_input(label = 'Next service max. mileage',
                                                            placeholder = 'Next service max. mileage?')
            ...
            if submitted:
                # Get latest ID from database
                id = lastID(url = '`carfleet`.`SERVICES`')
                query = "INSERT INTO `carfleet`.`SERVICES`(ID, VEHICLE_ID, DRIVER_ID, SERVICE_DATE, SERVICE_DETAILS, SERVICE_COSTS, SERVICE_MILEAGE_ON_SERVICE, SERVICE_MILEAGE_NEXTSERVICE, SERVICE_MILEAGE_NEXTSERVICE_MAX) VALUES (%s, '%s', '%s', '%s', '%s', %s, %s, %s, %s);" % (
                id, vehicle_id, driver_id, service_date, service_details, service_costs, service_mileage_on_service,
                service_mileage_nextservice, service_mileage_nextservice_max)
                run_query(query)
                conn.commit()
                st.experimental_rerun()
        ...
        ## tab `Trips`
            ...
            if submitted:
                # Get latest ID from database
                id = lastID(url = '`carfleet`.`TRIPS`')
                query = "INSERT INTO `carfleet`.`TRIPS`(ID, VEHICLE_ID, DRIVER_ID, TRIP_DATE, TRIP_DESCRIPTION, TRIP_COMMENTS, TRIP_TIME_OUT, TRIP_TIME_IN, TRIP_OPEN_MILEAGE, TRIP_CLOSE_MILEAGE, TRIP_DISTANCE) VALUES (%s, '%s', '%s', '%s', '%s', '%s', '%s', '%s', %s, %s, %s);" % (
                id, vehicle_id, driver_id, trip_date, trip_description, trip_comments, trip_time_out, trip_time_in,
                trip_open_mileage, trip_close_mileage, trip_distance)
                run_query(query)
                conn.commit()
                st.experimental_rerun()
        ...
        ## tab `Vehicles`
            ...
            if submitted:
                # Get latest ID from database
                id = lastID(url = '`carfleet`.`VEHICLES`')
                query = "INSERT INTO `carfleet`.`VEHICLES`(ID, VEHICLE_ID, VEHICLE_PLATE_NUMBER, VEHICLE_TYPE, VEHICLE_BRAND, VEHICLE_MODEL, VEHICLE_SEATS, VEHICLE_FUEL_TYPE, VEHICLE_FUEL_CAPACITY, VEHICLE_COLOUR, VEHICLE_CHASIS_NUMBER, VEHICLE_MANUFACTURE_YEAR, VEHICLE_PURCHASE_DATE, VEHICLE_PURCHASE_PRICE, VEHICLE_DISPOSITION_YEAR, VEHICLE_COF_EXPIRY_DATE, VEHICLE_VENDOR, VEHICLE_DUTY, VEHICLE_COST_KM) VALUES (%s, '%s', '%s', '%s', '%s', '%s', %s, '%s', %s, '%s', '%s', '%s', '%s', %s, '%s', '%s', '%s', %s, %s);" % (
                id, vehicle_id, vehicle_plate_number, vehicle_type, vehicle_brand, vehicle_model, vehicle_seats,
                vehicle_fuel_type, vehicle_fuel_capacity, vehicle_colour, vehicle_chasis_number,
                vehicle_manufacture_year, vehicle_purchase_date, vehicle_purchase_price, vehicle_disposition_year,
                vehicle_cof_expiry_date, vehicle_vendor, vehicle_duty, vehicle_cost_km)
                run_query(query)
                conn.commit()
    ...
    if (f"{chosen_id}" == '1'):
        ...
        if st.button('Export Drivers data to Excel document'):
            export_excel('Drivers', 'K',
                         [{'header': 'EMPLOYEE_NO'}, {'header': 'DRIVER_ID'}, {'header': 'DRIVER_FORENAME'},
                          {'header': 'DRIVER_SURNAME'}, {'header': 'DRIVER_NATIONAL_ID'},
                          {'header': 'DRIVER_MOBILE_NO'}, {'header': 'DRIVER_LICENSE_NO'},
                          {'header': 'DRIVER_LICENSE_CLASS'}, {'header': 'DRIVER_LICENSE_EXPIRY_DATE'},
                          {'header': 'DRIVER_PSV_BADGE'}, {'header': 'DRIVER_NOTES'}, ],
                         int(len(databank_drivers_excel) + 1), databank_drivers_excel)
        ...
        with st.expander('Driver profiles', expanded = False):
            ...
            for i in range(len(databank_drivers)):
                ...
                drivers_desc.append(
                    databank_drivers._get_value(i + 1, 'DRIVER_FORENAME') + ' ' + databank_drivers._get_value(i + 1,
                                                                                                              'DRIVER_SURNAME'))

            # Show selectable images
            drivers_option = image_select(label = 'Which Driver to choose?', images = images, captions = drivers_desc,
            ...
            sty.scrollableTextbox('Name: ' + databank_drivers._get_value(drivers_option + 1,
                                                                         'DRIVER_FORENAME') + ' ' + databank_drivers._get_value(
                drivers_option + 1, 'DRIVER_SURNAME') + '; Driver ID: ' + databank_drivers._get_value(
                drivers_option + 1, 'DRIVER_ID') + '; Mobile number: ' + databank_drivers._get_value(drivers_option + 1,
                                                                                                     'DRIVER_MOBILE_NO') + '; License number: ' + databank_drivers._get_value(
                drivers_option + 1, 'DRIVER_LICENSE_NO') + '; License class: ' + databank_drivers._get_value(
                drivers_option + 1, 'DRIVER_LICENSE_CLASS') + '; Expiry date: ' + str(
                databank_drivers._get_value(drivers_option + 1,
                                            'DRIVER_LICENSE_EXPIRY_DATE')) + '; PSV Badge: ' + databank_drivers._get_value(
                drivers_option + 1, 'DRIVER_PSV_BADGE') + '; Driver notes: ' + databank_drivers._get_value(
                drivers_option + 1, 'DRIVER_NOTES'), height = 56, border = True)


            ## Export driver profile to Excel Makro file
            if st.button('Export Driver data to Excel document'):
                export_excel('Drivers', 'A', [{'header': 'DRIVER'}, ],
                             int(len(databank_drivers_excel.iloc[drivers_option]) + 1),
                             databank_drivers_excel.iloc[drivers_option],
                             image = databank_drivers._get_value(drivers_option + 1, 'DRIVER_IMAGE'), image_pos = 'C1')
    ...
    ## Data analysis for `Fuel`
        ...
        if st.button('Export Fuel data to Excel document'):
            export_excel('Fuel', 'I', [{'header': 'VEHICLE_ID'}, {'header': 'DRIVER_ID'}, {'header': 'FUEL_AMOUNT'},
                                       {'header': 'FUEL_COST'}, {'header': 'FUEL_TYPE'}, {'header': 'FUEL_DATE'},
                                       {'header': 'FUEL_DISTANCE'}, {'header': 'FUEL_SHORTAGE'},
                                       {'header': 'COST_CENTRE'}, ], int(len(databank_fuel) + 1), databank_fuel)
        ...
        with st.expander('Fuel statistics', expanded = False):
            ...
            if (selected_vehicle == 'All vehicles'):
                ...
                for i in range(len(vehicles)):
                    query = "SELECT ID, VEHICLE_ID, FUEL_AMOUNT, FUEL_DISTANCE FROM `carfleet`.`FUEL` WHERE VEHICLE_ID = %s;" % (
                    vehicles[i])
                    rows = run_query(query)
                    i = 0
                    fuel = 0.0
                    ...
                    df = pd.DataFrame([[row[1], round((fuel / i), 2)]],
                                      columns = ['Vehicle ID', 'Average Fuel Consumption'])
            ...
            ## Show fuel consumption of one vehicle
                ...
                query = "SELECT ID, VEHICLE_ID, FUEL_AMOUNT, FUEL_DISTANCE, FUEL_DATE FROM `carfleet`.`FUEL` WHERE VEHICLE_ID = %s;" % (
                    vehicles)
                rows = run_query(query)
                fuel = 0.0
                for row in rows:
        ...
        with st.expander('Fuel Report', expanded = False):
            ...
            for i in range(len(databank_vehicles)):
                for x in range(len(vehicles_list)):
                    if databank_vehicles._get_value(i + 1, 'VEHICLE_ID') == vehicles_list[x]:
                        ...
                        vehicles_desc.append('Vehicle ID: ' + databank_vehicles._get_value(i + 1,
                                                                                           'VEHICLE_ID') + ' (' + databank_vehicles._get_value(
                            i + 1, 'VEHICLE_PLATE_NUMBER') + ')')

            # Show selectable images
            vehicle = image_select(label = 'Which Vehicle to choose?', images = images, captions = vehicles_desc,
                                   index = 0, return_value = 'index')
            ...
            query = "SELECT ID, VEHICLE_ID, FUEL_AMOUNT, FUEL_DISTANCE, FUEL_DATE FROM carfleet.FUEL WHERE FUEL_DATE BETWEEN DATE_SUB(NOW(), INTERVAL %s WEEK) AND NOW() AND VEHICLE_ID = '%s';" % (
            weeks, vehicle_id[vehicle])
            rows = run_query(query)
            for row in rows:
                fuel_avg = round(((row[2] * 100) / row[3]), 2)
                date = str(row[4])[:10]
                df = pd.DataFrame([[row[1], date, fuel_avg]],
                                  columns = ['Vehicle ID', 'Date', 'Average Fuel Consumption'])
            ...
            query = "SELECT ID, VEHICLE_ID, FUEL_COST, FUEL_DATE FROM carfleet.FUEL WHERE FUEL_DATE BETWEEN DATE_SUB(NOW(), INTERVAL %s WEEK) AND NOW() AND VEHICLE_ID = '%s';" % (
            weeks, vehicle_id[vehicle])
            rows = run_query(query)
            for row in rows:
            ...
            query = "SELECT fue.ID, fue.VEHICLE_ID, fue.FUEL_AMOUNT, fue.FUEL_DATE, vec.VEHICLE_FUEL_CAPACITY FROM carfleet.FUEL AS fue LEFT JOIN carfleet.VEHICLES AS vec ON vec.VEHICLE_ID = fue.VEHICLE_ID WHERE fue.FUEL_DATE BETWEEN DATE_SUB(NOW(), INTERVAL %s WEEK) AND NOW() AND fue.VEHICLE_ID = '%s';" % (
            weeks, vehicle_id[vehicle])
            rows = run_query(query)
            for row in rows:
                fuel_amount = round(row[2], 1)
                date = str(row[3])[:10]
                df = pd.DataFrame([[row[1], date, fuel_amount, row[4]]],
                                  columns = ['Vehicle ID', 'Date', 'Fuel Amount', 'Fuel max. Capacity'])
            ...
            if st.button('Export Fuel Report to Excel document'):
                ...
                report_image = databank_vehicles._get_value(vehicle + 1, 'VEHICLE_IMAGE')
                export_excel('Avg. Fuel Consumption', 'B',
                             [{'header': 'Date'}, {'header': 'Average Fuel Consumption'}, ],
                             int(len(report_fuel_consumption_average) + 1), report_fuel_consumption_average,
                             'Fuel Cost per Litre', 'B', [{'header': 'Date'}, {'header': 'Fuel Cost'}, ],
                             int(len(report_fuel_price_litre) + 1), report_fuel_price_litre,
                             'Fuel max. Capacity', 'C',
                             [{'header': 'Date'}, {'header': 'Fuel Amount'}, {'header': 'Fuel max. Capacity'}, ],
                             int(len(report_fuel_max_cap) + 1), report_fuel_max_cap,
    ...
    ## Data analysis for `Insurances`
        ...
        if st.button('Export Insurances data to Excel document'):
            export_excel('Insurances', 'E',
                         [{'header': 'VEHICLE_ID'}, {'header': 'INSURANCE_DETAILS'}, {'header': 'INSURANCES_TYPE'},
                          {'header': 'INSURANCE_START_DATE'}, {'header': 'INSURANCE_EXPIRY_DATE'}, ],
                         int(len(databank_insurances) + 1), databank_insurances)
    ...
    ## Data analysis for `Repairs`
        ...
        if st.button('Export Repairs data to Excel document'):
            export_excel('Repairs', 'H',
                         [{'header': 'VEHICLE_ID'}, {'header': 'REPAIR_DETAILS'}, {'header': 'REPAIR_DATE'},
                          {'header': 'REPAIR_COSTS'}, {'header': 'REPAIR_SPARE_PARTS'}, {'header': 'REPAIR_DOWN_TIME'},
                          {'header': 'REPAIR_CENTRE_PERSON'}, {'header': 'COST_CENTRE'}, ],
                         int(len(databank_repairs) + 1), databank_repairs)
        ...
        with st.expander('Repairs statistics', expanded = False):
            ...
            if (selected_vehicle == 'All vehicles'):
                ...
                for i in range(len(vehicles)):
                    query = "SELECT ID, VEHICLE_ID, REPAIR_COSTS FROM `carfleet`.`REPAIRS` WHERE VEHICLE_ID = %s;" % (
                    vehicles[i])
                    rows = run_query(query)
                    costs = 0.0
                    for row in rows:
            ...
            ## Show repair costs per incident
                ...
                query = "SELECT ID, VEHICLE_ID, REPAIR_DETAILS, REPAIR_COSTS FROM `carfleet`.`REPAIRS` WHERE VEHICLE_ID = %s;" % (
                    vehicles)
                rows = run_query(query)
                for row in rows:
    ...
    ## Data analysis for `Services`
        ...
        if st.button('Export Services data to Excel document'):
            export_excel('Services', 'H',
                         [{'header': 'VEHICLE_ID'}, {'header': 'DRIVER_ID'}, {'header': 'SERVICE_DATE'},
                          {'header': 'SERVICE_DETAILS'}, {'header': 'SERVICE_COSTS'},
                          {'header': 'SERVICE_MILEAGE_ON_SERVICE'}, {'header': 'SERVICE_MILEAGE_NEXTSERVICE'},
                          {'header': 'SERVICE_MILEAGE_NEXTSERVICE_MAX'}, ], int(len(databank_services) + 1),
                         databank_services)
    ...
    ## Data analysis for `Trips`
        ...
        if st.button('Export Trips data to Excel document'):
            export_excel('Trips', 'J', [{'header': 'VEHICLE_ID'}, {'header': 'DRIVER_ID'}, {'header': 'TRIP_DATE'},
                                        {'header': 'TRIP_DESCRIPTION'}, {'header': 'TRIP_COMMENTS'},
                                        {'header': 'TRIP_TIME_OUT'}, {'header': 'TRIP_TIME_IN'},
                                        {'header': 'TRIP_OPEN_MILEAGE'}, {'header': 'TRIP_CLOSE_MILEAGE'},
                                        {'header': 'TRIP_DISTANCE'}, ], int(len(databank_trips) + 1), databank_trips)
    ...
    ## Data analysis for `Vehicles`
        ...
        if st.button('Export Vehicles data to Excel document'):
            export_excel('Vehicles', 'R',
                         [{'header': 'VEHICLE_ID'}, {'header': 'VEHICLE_PLATE_NUMBER'}, {'header': 'VEHICLE_TYPE'},
                          {'header': 'VEHICLE_BRAND'}, {'header': 'VEHICLE_MODEL'}, {'header': 'VEHICLE_SEATS'},
                          {'header': 'VEHICLE_FUEL_TYPE'}, {'header': 'VEHICLE_FUEL_CAPACITY'},
                          {'header': 'VEHICLE_COLOUR'}, {'header': 'VEHICLE_CHASIS_NUMBER'},
                          {'header': 'VEHICLE_MANUFACTURE_YEAR'}, {'header': 'VEHICLE_PURCHASE_DATE'},
                          {'header': 'VEHICLE_PURCHASE_PRICE'}, {'header': 'VEHICLE_DISPOSITION_YEAR'},
                          {'header': 'VEHICLE_COF_EXPIRY_DATE'}, {'header': 'VEHICLE_VENDOR'},
                          {'header': 'VEHICLE_DUTY'}, {'header': 'VEHICLE_COST_KM'}, ],
                         int(len(databank_vehicles_excel) + 1), databank_vehicles_excel)
        ...
        with st.expander('Vehicles profiles', expanded = False):
            ...
            for i in range(len(databank_vehicles)):
                ...
                vehicles_desc.append('Vehicle ID: ' + databank_vehicles._get_value(i + 1,
                                                                                   'VEHICLE_ID') + ' (' + databank_vehicles._get_value(
                    i + 1, 'VEHICLE_PLATE_NUMBER') + ') Brand: ' + databank_vehicles._get_value(i + 1,
                                                                                                'VEHICLE_BRAND') + ' ' + databank_vehicles._get_value(
                    i + 1, 'VEHICLE_MODEL'))

            # Show selectable images
            vehicles_option = image_select(label = 'Which Vehicle to choose?', images = images,
                                           captions = vehicles_desc, index = 0, return_value = 'index')
            ...
            sty.scrollableTextbox('Vehicle ID: ' + databank_vehicles._get_value(vehicles_option + 1,
                                                                                'VEHICLE_ID') + '; Plate Number: ' + databank_vehicles._get_value(
                vehicles_option + 1, 'VEHICLE_PLATE_NUMBER') + '; Type: ' + databank_vehicles._get_value(
                vehicles_option + 1, 'VEHICLE_TYPE') + '; Brand: ' + databank_vehicles._get_value(vehicles_option + 1,
                                                                                                  'VEHICLE_BRAND') + '; Model: ' + databank_vehicles._get_value(
                vehicles_option + 1, 'VEHICLE_MODEL') + '; Seats: ' + str(
                databank_vehicles._get_value(vehicles_option + 1,
                                             'VEHICLE_SEATS')) + '; Fuel Type: ' + databank_vehicles._get_value(
                vehicles_option + 1, 'VEHICLE_FUEL_TYPE') + '; Fuel Capacity: ' + str(
                databank_vehicles._get_value(vehicles_option + 1,
                                             'VEHICLE_FUEL_CAPACITY')) + '; Colour: ' + databank_vehicles._get_value(
                vehicles_option + 1, 'VEHICLE_COLOUR') + '; Chasis Number: ' + databank_vehicles._get_value(
                vehicles_option + 1, 'VEHICLE_CHASIS_NUMBER') + '; Manufacture Year: ' + str(
                databank_vehicles._get_value(vehicles_option + 1,
                                             'VEHICLE_MANUFACTURE_YEAR')) + '; Purchase Date: ' + str(
                databank_vehicles._get_value(vehicles_option + 1,
                                             'VEHICLE_PURCHASE_DATE')) + '; Purchase Price: ' + str(
                databank_vehicles._get_value(vehicles_option + 1,
                                             'VEHICLE_PURCHASE_PRICE')) + '; Disposition Year: ' + str(
                databank_vehicles._get_value(vehicles_option + 1, 'VEHICLE_DISPOSITION_YEAR')) + '; COF Expiry: ' + str(
                databank_vehicles._get_value(vehicles_option + 1,
                                             'VEHICLE_COF_EXPIRY_DATE')) + '; Vendor: ' + databank_vehicles._get_value(
                vehicles_option + 1, 'VEHICLE_VENDOR') + '; Duty: ' + str(
                databank_vehicles._get_value(vehicles_option + 1, 'VEHICLE_DUTY')) + '; Cost KM: ' + str(
                databank_vehicles._get_value(vehicles_option + 1, 'VEHICLE_COST_KM')), height = 56, border = True)


            ## Export vehicle profile to Excel Makro file
            if st.button('Export Vehicle data to Excel document'):
                export_excel('Vehicles', 'A', [{'header': 'VEHICLE'}, ],
                             int(len(databank_vehicles_excel.iloc[vehicles_option]) + 1),
                             databank_vehicles_excel.iloc[vehicles_option],
                             image = databank_vehicles._get_value(vehicles_option + 1, 'VEHICLE_IMAGE'),
                             image_pos = 'C1')
        ...
        if st.button('Database Exel Export (all tables)'):
            ...
            export_excel('Drivers', 'K',
                         [{'header': 'EMPLOYEE_NO'}, {'header': 'DRIVER_ID'}, {'header': 'DRIVER_FORENAME'},
                          ...
                          {'header': 'DRIVER_PSV_BADGE'}, {'header': 'DRIVER_NOTES'}, ],
                         int(len(databank_drivers_excel) + 1), databank_drivers_excel,
                         'Fuel', 'I', [{'header': 'VEHICLE_ID'}, {'header': 'DRIVER_ID'}, {'header': 'FUEL_AMOUNT'},
                                       {'header': 'FUEL_COST'}, {'header': 'FUEL_TYPE'}, {'header': 'FUEL_DATE'},
                                       {'header': 'FUEL_DISTANCE'}, {'header': 'FUEL_SHORTAGE'},
                                       {'header': 'COST_CENTRE'}, ], int(len(databank_fuel) + 1), databank_fuel,
                         'Insurances', 'E',
                         ...
                         [{'header': 'VEHICLE_ID'}, {'header': 'DRIVER_ID'}, {'header': 'SERVICE_DATE'},
                          ...
                          {'header': 'SERVICE_MILEAGE_NEXTSERVICE_MAX'}, ], int(len(databank_services) + 1),
                         databank_services,
                         'Trips', 'J', [{'header': 'VEHICLE_ID'}, {'header': 'DRIVER_ID'}, {'header': 'TRIP_DATE'},
                                        {'header': 'TRIP_DESCRIPTION'}, {'header': 'TRIP_COMMENTS'},
                                        {'header': 'TRIP_TIME_OUT'}, {'header': 'TRIP_TIME_IN'},
                                        {'header': 'TRIP_OPEN_MILEAGE'}, {'header': 'TRIP_CLOSE_MILEAGE'},
                                        {'header': '`TRIP_DISTANCE'}, ], int(len(databank_trips) + 1), databank_trips,

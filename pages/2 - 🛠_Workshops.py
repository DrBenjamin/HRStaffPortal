...
if check_password():
  ...
  index_workshop = st.selectbox(label = "Which workshop do you want to select?", options = range(len(workshops)), format_func = lambda x: workshops[x], index = index_workshop)
  ...
  if index_workshop > 0:
    ...
    with st.expander(label = '', expanded = True):
      ...
      with st.form('Workshop'):
        update = False
        title = st.text_input(label = 'Title', value = databank_workshop._get_value(index_workshop, 'WORKSHOP_TITLE'), disabled = not checkbox_val)
        if (databank_workshop._get_value(index_workshop, 'WORKSHOP_TITLE') != title):
          update = True
        desc = st.text_input(label = 'Description', value = databank_workshop._get_value(index_workshop, 'WORKSHOP_DESCRIPTION'), disabled = not checkbox_val)
        if (databank_workshop._get_value(index_workshop, 'WORKSHOP_DESCRIPTION') != desc):
          update = True
        facilitator = st.text_input(label = 'Facilitator', value = databank_workshop._get_value(index_workshop, 'WORKSHOP_FACILITATOR'), disabled = not checkbox_val)
        if (databank_workshop._get_value(index_workshop, 'WORKSHOP_FACILITATOR') != facilitator):
          update = True
        facilitator_email = st.text_input(label = 'Email address', value = databank_workshop._get_value(index_workshop, 'WORKSHOP_FACILITATOR_EMAIL'), disabled = not checkbox_val)
        if (databank_workshop._get_value(index_workshop, 'WORKSHOP_FACILITATOR_EMAIL') != facilitator_email):
          update = True
        date = st.date_input(label = 'Date', value = databank_workshop._get_value(index_workshop, 'WORKSHOP_DATE'), disabled = not checkbox_val)
        if (databank_workshop._get_value(index_workshop, 'WORKSHOP_DATE') != date):
          update = True
        duration = st.text_input(label = 'Duration', value = databank_workshop._get_value(index_workshop, 'WORKSHOP_DURATION'), disabled = not checkbox_val)

...
#keyword1 = ''
#keywords = []
#databank_handbook = pd.DataFrame(columns = ['ID_Index', 'ID', 'HANDBOOK_ID', 'CATEGORY_ID', 'CATEGORY_SUB_ID', 'HANDBOOK_KEYWORD1', 'HANDBOOK_KEYWORD2', 'HANDBOOK_KEYWORD3', 'HANDBOOK_KEYWORD4', 'HANDBOOK_KEYWORD5', 'HANDBOOK_TEXT', 'HANDBOOK_HITS'])
...
with st.form('Input', clear_on_submit = True):
  ...
  with col1:
    ...
    st.write('Where do I get the login username and password for the HR Staff Portal from?')
          ...
          if (x == 0):
            ...
            for i in range(x):
              if (databank_handbook.iloc[x][11] <= databank_handbook.iloc[i][11]):
                ...
                if (i == x - 1):
                  handbook = databank_handbook._get_value(databank_handbook.iloc[x][0], 'HANDBOOK_TEXT')
                  st.write('Running Nr. x: ', handbook)
                  handbook_id = databank_handbook._get_value(databank_handbook.iloc[x][0], 'HANDBOOK_ID')
                  st.write('Handbook ID: ', hanbdbook_id)
                  st.session_state['handbook_id'] = handbook_id
...
if (len(answer) > 0):
  ...
  if (answer != 'Unfortunately we have not found the answer to your question.'):
    ...
    if st.button(label = 'Yes'):
      ...
      # Response to user
      st.write('Thanks for your feedback, it was recorded.')
    else:
